...
build pattern = resTable
  where
    ...
    pred a b c =  len == a ||
              alphabetTable resTable ! b /= alphabetTable resTable ! c
    ...
    genJump lastMPJump i =
      let ch = alphabetTable resTable ! i
          findJ j
            | j == -2 = -2
...
matchOne table j s
  ...
  where
    hasFind = j < 0 || j < len table && s == alphabetTable table ! j
